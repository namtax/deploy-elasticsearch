...
task :upload => :setup do
  @server ||= Provision::Server.new @args
  @server.scp "bootstrap.sh #{@args[:node_json]} solo.rb", :path => '/tmp'
  exit(1) unless $?.success?
end
...
task :setup do
  ...
  @args = {}
  @args[:name]                  = ENV['NAME'] || name || 'elasticsearch-01'
  @args[:node_json]             = node_json
  @args[:aws_ssh_key_id]        = ENV['AWS_SSH_KEY_ID'] || 'pstam-keypair'
  ...
  @args[:aws_groups]            = ENV['GROUP'] || aws_group || 'elasticsearch'
  @args[:aws_flavor]            = ENV['FLAVOR'] || 'm1.large'
